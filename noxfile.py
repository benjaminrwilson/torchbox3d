...
def _setup(session: Session) -> None:
    """Install `torchbox3d` into a virtual environment.
...
def pytest(session: Session) -> None:
    ...
    _setup(session)
    session.install(*env)
    session.run("pytest", "tests", "--cov", "src/torchbox3d")
